...
const h1 = document.querySelector('#quote-div h1');
const p = document.querySelector('#quote-div p');
...
function getRandomQuote(){
    let randomQuoteIndex = Math.floor(Math.random() * arrayOfQuotes.length);
...
function setRandomQuote(){
    ...
    while(randomQuoteIndex === currentQuote ){
        randomQuoteIndex = getRandomQuote();
    }
    
    h1.textContent = arrayOfQuotes[randomQuoteIndex][0];
    p.textContent = arrayOfQuotes[randomQuoteIndex][1];
...
btn.addEventListener('click', setRandomQuote);
